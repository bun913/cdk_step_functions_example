...
export class Ecr {
  ...
  public createResources() {
    const okRepo = new Repository(this.scope, 'OKRepo', {
      removalPolicy: RemovalPolicy.DESTROY,
    });
    const ngRepo = new Repository(this.scope, 'NGRepo', {
      removalPolicy: RemovalPolicy.DESTROY,
    });
    ...
    const ecrDeploySet = [
      {
        name: 'okImage',
        imgSource: okImage.imageUri,
        repoUri: okRepo.repositoryUri,
      },
      {
        name: 'ngImage',
        imgSource: ngImage.imageUri,
        repoUri: ngRepo.repositoryUri,
      },
    ];
